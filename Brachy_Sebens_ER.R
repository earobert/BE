...
a_perfood <- 0.25
b_permass <- 0.0145
a <- 0.553 # was 0.25 # intake
b <- 0.027 # was 0.0145 # cost
eggs <- 0.11 # eggs
...
Tmult_int <- if(temp<critical_T_intake){
  Tmult_cost
  } else {
    Tmult_cost - Tmult_cost*(temp-critical_T_intake)/(max_T_intake-critical_T_intake)
  }
...
snapshot_time <- 500
cost[snapshot_time]
mass_ww[snapshot_time]
reproduction[snapshot_time]
