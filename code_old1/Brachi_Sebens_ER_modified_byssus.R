# ORIGINAL Brachi MODEL FROM Sebens et al, modified to go with data, parameters still brachi ####
# compare the different growth models
...
# MODIFICATION_Past ultimate size with byssus ####
# Now run to estimate growth a different byssal thread allocations, 10%, 1%, 0%. 
# Similar to EOS, check that it's on the correct side of the range. 
# 10% byssus ####
# Longer time for optimal EOS ####
max_time <- 1700
...
for(i in 1:(max_time)){
  ...
  intake[i] <- a*(mass_ww[i]^d)
  byssus[i] <- .1*b*(mass_ww[i]^e)
  cost[i] <- b*(mass_ww[i]^e)+byssus[i]
  #E_surplus[i] <- a*(mass_ww[i]^d)-b*(mass_ww[i]^e) #in J, Is this necessary?
  E_surplus[i] <- intake[i]-cost[i]
...
# Plot on one graph ####
par(mfrow = c(1,1))
df.0_perc<-df.0_perc[df.0_perc$E_surplus>0,]
norm_fact <- max(df.0_perc$mass_ww)
plot(df.0_perc$mass_ww/norm_fact*100, df.0_perc$E_surplus, 
     type = "l", xlim = c(0,100), 
     ylim = c(0,50),
     yaxs = "i",
     xaxs = "i",
     ylab = "E_surplus",
     xlab = "% of ultimate size",
     lty = 2)
lines(df.1_perc$mass_ww/norm_fact*100, df.1_perc$E_surplus)
lines(df.4_perc$mass_ww/norm_fact*100, df.4_perc$E_surplus)
lines(df.10_perc$mass_ww/norm_fact*100, df.10_perc$E_surplus)
