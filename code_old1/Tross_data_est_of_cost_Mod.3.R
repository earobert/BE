...
#===============================================#
# Model 3, in the format of Model 2. Gamma is constrained, but cancels out
# MLE estimation of cost of byssus given growth and frequency of manipulation####
...
mod.1 <- function(params, season) {
  ...
  frequency_cut <- rep(-99, length = nrow(df))
  frequency_cut[df$treatment == "never"] <- .25
  frequency_cut[df$treatment == "weekly"] <- 1
  frequency_cut[df$treatment == "daily"] <- 7
  ...
  byssus_induced <- b*(mass_ww^e)*frequency_cut*cost_per_thread #this is still called "cost per thread," but it is really cost of cutting the byssus
  reproduction <- 0
...
a.plot.NLL1 <- function(params, season) {
  ...
  p.1 <- lm(growth_tissue~ frequency_cut)
  plot(x = frequency_cut, y = growth_tissue, 
       col = as.numeric(df$treatment)+26,
       pch = 19,
       xlab = "Frequency of thread manipulation \n (per week)",
       ylab = "Observed growth (gWW)"
  )
  abline(p.1)
  
  p.2 <- lm(growth_tissue~model.predG_g)
  plot(x = model.predG_g, y = growth_tissue, 
       col = as.numeric(df$treatment)+26,
       pch = 19,
       ylim = c(0,.3), xlim = c(0,.3),
       xlab = "Predicted growth (gWW)",
       ylab = "Observed growth (gWW)"
  )
  abline(p.2)
