...
# This script plots log relationships rather than raw growth relationships
# This script also includes an mle2 version of optimization - 
#   it works the same as using optim, but takes up more space. It doesn't generate any warnings unlike optim. 
#===============================================#
...
#For troubleshooting:
params <- c(food = 4, cost_induced_byssus = 0.2, sigma = .05)
season <- "Autumn"
...
a.est.NLL1 <- function(params, season) {
  ...
  model.predG_g <- out$model.predG_g
  sigma <- params[3]
  NLL <- -sum(dnorm(x=growth_tissue+1, mean=model.predG_g+1, sd=sigma, log=TRUE))
  return(NLL)
}
...
# One alternative way of estimating using mle2. This gives the same answer with no warnings. 
library(bbmle)
a.mle2.NLL.Spring <- function(food, cost_induced_byssus, sigma) {
  params <- c(food, cost_induced_byssus, sigma)
  # if(season_code == 1){
  #   season = "Spring"
  # } else {
  #   season = "Autumn"
...
a.mle2.NLL.Autumn <- function(food, cost_induced_byssus, sigma) {
  params <- c(food, cost_induced_byssus, sigma)
  # if(season_code == 1){
  #   season = "Spring"
  # } else {
...
mleBE.Autumn <- mle2(minuslogl=a.mle2.NLL.Autumn, 
                     method = "L-BFGS-B", 
                     lower = c(food = 0, cost_induced_byssus = 0.001, sigma = .0001), 
                     upper = c(food = 5, cost_induced_byssus = 0.1, sigma = 100),
                     start = list(food = 3, cost_induced_byssus = 0.01, sigma = 1), 
)
mleBE.Autumn

mleBE.Spring <- mle2(minuslogl=a.mle2.NLL.Spring, 
                     method = "L-BFGS-B", 
                     lower = c(food = 0, cost_induced_byssus = 0.001, sigma = .0001), 
                     upper = c(food = 5, cost_induced_byssus = 0.1, sigma = 100),
                     start = list(food = 3, cost_induced_byssus = 0.01, sigma = 1)
...
a.plot.NLL1 <- function(params, season) {
  ...
  p.2 <- lm(log(model.predG_g+1)~log(growth_tissue+1))
  plot(x = log(growth_tissue+1), y = log(model.predG_g+1), 
       col = as.numeric(df$treatment)+26,
       pch = 19,
       #ylim = log(c(0,.3)+1), xlim = log(c(0,.3)+1),
...
#a.plot.NLL1(par=c(food = 2.5, cost_per_byssus = .3, sigma = .2), season = "Spring")
