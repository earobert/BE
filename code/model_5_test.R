...
# This is the main working model right now BUT using 'fake data' to test the model. 
...
#For troubleshooting:
params <- c(food = 4, cost_induced_byssus = 0.02, sigma = 10)
season <- "Autumn"
...
practice.est <- optim(fn=a.est.NLL1, par=c(food = 4, cost_induced_byssus = 0.03, sigma = 10), season = "Autumn") # par are the starting values
#Spring.est <- optim(fn=a.est.NLL1, par=c(food = 3, cost_induced_byssus = 0.01, sigma = 100), season = "Spring") # par are the starting values

#I'm getting the same food and byssus costs that I used to make the data! Not sure about what sigma I used...?
# practice.est
# $par
# food cost_induced_byssus               sigma 
# 3.000000e+00        1.000000e-01        3.819532e-11 
# 
# $value
# [1] -609.8384
# 
...
a.plot.NLL1 <- function(params, season) {
  ...
  # p.1 <- lm(growth_tissue~ thread_num)
  # plot(x = thread_num, y = growth_tissue, 
...
a.plot.NLL1(par=c(food = 3, cost_per_byssus = .2, sigma = 100), season = "Autumn")
a.plot.NLL1(par=c(food = practice.est$par[1], cost_per_byssus = practice.est$par[2], sigma = practice.est$par[3]), season = "Autumn")

# The second plot with the estimated parameters is 1:1. This means the model predicts the model parameters correctly. 
